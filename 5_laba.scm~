  ...
  (if (= (vector-length program) index)
      ...
      (let ((word (vector-ref program index)))
        (cond
          ...
          ((equal? word 'if) (interpreter (if (zero? (car stack)) (+ (word-index 'endif program index) 1) (+ index 1)) (cdr stack) return-stack definitions))
          ((equal? word 'endif) (interpreter (+ index 1) stack return-stack definitions))
          ((equal? word 'while) (if (zero? (car stack))
                                    (interpreter (+ (word-index 'wend program index) 1) stack return-stack definitions)
          ...
          ((equal? word 'for) (if (<= (car stack) (cadr stack))
                                  (interpreter (+ index 1) stack (cons index return-stack) definitions)
                                  (interpreter (+ (word-index 'next program index) 1) (cddr stack) return-stack definitions)))
          ((equal? word 'next) (interpreter (car return-stack) (cons (+ (car stack) 1) (cdr stack)) (cdr return-stack) definitions))
